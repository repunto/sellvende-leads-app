export default function WaTourSelectorModal({ waModal, setWaModal, finishWaClick }) {
            ...
            <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 460, padding: 0, overflow: 'hidden' }}>
                <div style={{ padding: '18px 22px', background: 'linear-gradient(135deg, #25D366, #128C7E)', color: 'white' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h3 style={{ margin: 0, fontSize: '1.05rem', display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            Seleccionar Tour para WhatsApp
                        </h3>
                    ...
                    <p style={{ margin: '4px 0 0', fontSize: '0.82rem', opacity: 0.85 }}>Elige la variante del tour para incluir el precio correcto</p>
                </div>
                <div style={{ padding: 20 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 320, overflowY: 'auto' }}>
                        {(waModal.matchingTours || []).map(tour => (
                            <button key={tour.id}
                                onClick={() => finishWaClick(waModal.lead, waModal.baseMsg, tour)}
                                style={{
                                ...
                                <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>{tour.nombre}</div>
                                <div style={{ fontSize: '0.8rem', color: '#25D366', marginTop: 2 }}>
                                    ${tour.precio_usd} USD · {tour.duracion_dias} días
                                </div>
                            </button>
                        ))}
                    </div>
