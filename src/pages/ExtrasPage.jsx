...
export default function ExtrasPage() {
    ...
    useEffect(() => { if (agencia?.id) loadExtras() }, [agencia?.id])
    ...
    async function loadExtras() {
        ...
        if (!error && data) setExtras(data)
        setLoading(false)
    }

    async function handleInlineUpdate(id, field, value) {
        const updatedList = extras.map(item => {
            if (item.id === id) {
    ...
    async function handleDelete(id) {
        setConfirmDialog({
            ...
            message: '¿Eliminar este extra/up-sell? Esta acción no se puede deshacer.',
            danger: true,
    ...
    return (
        <>
            ...
            <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <h1 className="page-title">Extras / Up-sells</h1>
                    <p className="page-subtitle">Servicios adicionales, upgrades y complementos que puedes añadir a una venta para aumentar el ticket promedio.</p>
                </div>
            ...
            <div className="page-body">
                {loading ? (
                    ...
                    <div className="empty-state">
                        ...
                        <div className="empty-state-text">No tienes extras registrados</div>
                        <div className="empty-state-sub">Añade servicios y complementos para aumentar el valor de cada venta.</div>
                    </div>
                ) : (
                    <div className="table-container">
                        <table className="data-table" style={{ tableLayout: 'fixed', width: '100%' }}>
                            ...
                            <thead>
                                <tr>
                                    <th>Nombre del Extra / Up-sell</th>
                                    <th>Precio (USD)</th>
                                    <th>Costo Interno</th>
                                    <th>Margen</th>
                                    <th>Estado</th>
