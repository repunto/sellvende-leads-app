...
export default function SeedTab({ showToast }) {
    ...
    async function handleSeed() {
        setConfirmDialog({
            ...
            message: '¿Cargar datos iniciales del Google Sheet?\n\nEsto insertará tours, operadores, leads y reservas de ejemplo. Si ya existen datos, se agregarán registros adicionales.',
            danger: true,
            confirmLabel: '📥 Cargar',
            onConfirm: async () => {
                ...
                try {
                    ...
                    if (res.errors.length === 0) {
                        showToast(`✅ Cargados: ${res.tours} tours, ${res.operadores} ops, ${res.leads} leads, ${res.reservas} revs, ${res.plantillas_email} tpls email, ${res.plantillas_wa} tpls wa`)
                    } else {
    ...
    return (
        <div className="card" style={{ maxWidth: 600 }}>
            ...
            <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.88rem', marginBottom: 20, lineHeight: 1.6 }}>
                ...
                <strong>14 tours</strong> (Salkantay, Inka Jungle, Inca Trail, Valle Sagrado, etc.),<br />
                <strong>6 operadores</strong>, <strong>10 leads de ejemplo</strong>, <strong>6 reservas</strong>,<br />
                ...
                <div style={{ marginTop: 20, padding: 16, background: 'var(--color-bg-elevated)', borderRadius: 8, border: '1px solid var(--color-border)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                        <div style={{ padding: 10, background: 'var(--color-bg-card)', borderRadius: 6, textAlign: 'center' }}>
                            <div style={{ fontSize: '1.5rem', fontWeight: 800 }}>{results.tours}</div>
                            <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)' }}>Tours</div>
                        </div>
                        <div style={{ padding: 10, background: 'var(--color-bg-card)', borderRadius: 6, textAlign: 'center' }}>
                            <div style={{ fontSize: '1.5rem', fontWeight: 800 }}>{results.operadores}</div>
                            <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)' }}>Operadores</div>
                        </div>
                        <div style={{ padding: 10, background: 'var(--color-bg-card)', borderRadius: 6, textAlign: 'center' }}>
                        ...
                        <div style={{ padding: 10, background: 'var(--color-bg-card)', borderRadius: 6, textAlign: 'center' }}>
                            <div style={{ fontSize: '1.5rem', fontWeight: 800 }}>{results.reservas}</div>
                            <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)' }}>Reservas</div>
                        </div>
                        <div style={{ padding: 10, background: 'var(--color-bg-card)', borderRadius: 6, textAlign: 'center' }}>
